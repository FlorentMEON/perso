...
public class Combattant {
    ...
    public void upendu(int i){
        Entite g = this.getGuerrier(i);

        int u = (int)(Math.random() * 10 + 1);

        int ancienPvMax = g.getPvmax();

        // On demande une augmentation brute
        g.setPvmax(ancienPvMax + u);

        // Calcul du gain réel appliqué
        int gainReel = g.getPvmax() - ancienPvMax;

        // On applique le même gain aux PV actuels
        g.setPointsDeVie(g.getPointsDeVie() + gainReel);

        System.out.println("Le combattant a maintenant : " + g.getPointsDeVie() + "/" + g.getPvmax());
    }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                switch (type) {
                    ...
                    case "humain":
                        e = new Personnage(nom, pvmax, damage);
                        break;

                    case "chaton":
                        e = new chaton(nom, pvmax, armure, damage);
                        break;
                    default:
